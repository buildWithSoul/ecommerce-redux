...
var _itemsActions = __webpack_require__(28);
...
store.dispatch((0, _itemsActions.postItem)([{
	id: 1,
...
store.dispatch((0, _itemsActions.deleteItem)({ id: 1 }));

store.dispatch((0, _itemsActions.updateItemTitle)({ id: 2, title: "this is the new title" }));
...
var _itemsReducers = __webpack_require__(25);
...
function itemsReducers() {
	...
	switch (action.type) {
		case "POST_ITEM":
			...
			break;
		case "DELETE_ITEM":
			var currentItems = [].concat(_toConsumableArray(state.items));
			var indexToDelete = currentItems.findIndex(function (item) {
				return item.id === action.payload.id;
			});
			return { items: [].concat(_toConsumableArray(currentItems.slice(0, indexToDelete)), _toConsumableArray(currentItems.slice(indexToDelete + 1))) };
			break;
		case "UPDATE_ITEM_TITLE":
			var currentItemsTitle = [].concat(_toConsumableArray(state.items));
			var indextoUpdate = currentItemsTitle.findIndex(function (item) {
				return item.id === action.payload.id;
			});
			var newItemtoUpdate = _extends({}, currentItemsTitle[indextoUpdate], {
				title: action.payload.title
			});
			return { items: [].concat(_toConsumableArray(currentItemsTitle.slice(0, indextoUpdate)), [newItemtoUpdate], _toConsumableArray(currentItemsTitle.slice(indextoUpdate + 1))) };
	}
	return state;
};
...
exports.updateItemTitle = updateItemTitle;
